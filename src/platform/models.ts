...
export async function loadModelRegistry(): Promise<ModelRegistryRecord[]> {
	...
	return parseModelRegistry(
		globalThis.localStorage?.getItem(MODEL_REGISTRY_STORAGE_KEY) ?? null,
	);
}
...
async function probeBrowserProvider(
	...
	try {
		...
		const models =
			kind === "ollama"
				? readOllamaModels(payload)
				: readOpenAiModelList(payload);
	...
	return payload.models
		.map((model): DiscoveredModel | null => {
			...
			return {
				...
				modifiedAt:
					typeof model.modified_at === "string" ? model.modified_at : null,
