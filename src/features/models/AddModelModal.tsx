...
import type { DiscoveredModel, DiscoveredModelSource, ModelCatalogEntry } from "@/platform/models";
...
interface AddModelModalProps {
  ...
  onFetchCatalog: (input: ModelRegistryInput) => Promise<ModelCatalogEntry[]>;
...
export function AddModelModal({
  ...
  const [input, setInput] = useState<ModelRegistryInput>(createDefaultModelInput("openai"));
  ...
  const groupedProviders = useMemo(
    () =>
      (["discovery", "built-in", "manual"] as const).map((group) => ({
        group,
        providers: modelProviderDefinitions.filter((definition) => definition.group === group),
      })),
    [],
  );
  ...
  function handleDiscoveredModel(source: DiscoveredModelSource, model: DiscoveredModel) {
  ...
  function handleDockerSource(source: DiscoveredModelSource) {
    setInput({
  ...
  async function loadCatalog() {
    ...
    try {
      ...
      setCatalog([]);
      setCatalogStatus(error instanceof Error ? error.message : "Catalog request failed.");
    } finally {
      setCatalogLoading(false);
    }
  }

  function handleCatalogEntry(entry: ModelCatalogEntry) {
    setInput({
      ...input,
    ...
    >
      <form className="model-add-form" onSubmit={handleSubmit}>
        <div className="model-add-layout">
          <section className="model-source-list" aria-label="Model source">
            {groupedProviders.map(({ group, providers }) => (
              <div className="connect-source-group" key={group}>
                <div className="connect-source-group__label">{groupLabels[group]}</div>
                <p className="model-source-group__hint">{groupDescriptions[group]}</p>
          ...
          <div
            className={
              shouldShowCatalog ? "model-add-config" : "model-add-config model-add-config--single"
            }
          >
            <div className="model-add-main">
              <section className="model-config-section">
                <header className="model-section-header">
                  ...
                  <Badge tone={selectedDefinition.requiresAuth ? "warning" : "neutral"}>
                    {selectedDefinition.requiresAuth ? "Auth" : "No auth"}
                  </Badge>
                </header>

                <div className="model-form-grid">
                  <label className="field-stack" htmlFor="model-name">
                    <span>Name</span>
                    <Input
                      id="model-name"
                  ...
                  <label className="field-stack" htmlFor="model-id">
                    <span>Model ID</span>
                    <Input
                      id="model-id"
                      onChange={(event) => setInput({ ...input, modelId: event.target.value })}
                  ...
                  {input.providerKind === "manual-cli" ? (
                    <>
                      <label className="field-stack" htmlFor="command">
                        <span>Command</span>
                        <Input
                          id="command"
                          onChange={(event) =>
                      ...
                      <label className="field-stack" htmlFor="args">
                        <span>Arguments</span>
                        <Input
                          id="args"
                          onChange={(event) => setInput({ ...input, args: event.target.value })}
                    ...
                    <label className="field-stack model-form-grid__wide" htmlFor="base-url">
                      <span>Base URL</span>
                      <Input
                        id="base-url"
                  ...
                  <div className="auth-config">
                    ...
                    <div className="auth-mode-row">
                      {(["env", "local", "none"] as const).map((type) => (
                        <button
                          className={
                            input.auth.type === type ? "auth-mode is-selected" : "auth-mode"
                          }
                          key={type}
                          onClick={() => setAuthType(type)}
                          type="button"
                        >
                          {type === "env" ? "Env var" : type === "local" ? "Local key" : "None"}
                        </button>
                      ))}
                    </div>
                    {input.auth.type === "env" ? (
                ...
                <section className="model-config-section">
                  ...
                  <div className="model-discovery-list">
                    {selectedDiscoverySources.length === 0 ? (
                      <p className="model-muted-line">No {selectedDefinition.label} results yet.</p>
                    ) : null}
                        ...
                        {source.kind === "docker" ? (
                          <div className="model-docker-row">
                            ...
                            <Button
                              ...
                              onClick={() => handleDockerSource(source)}
                              size="sm"
                            >
                              Use
                            </Button>
                          ...
                          source.models.slice(0, 8).map((model) => (
                            <div className="model-catalog-row" key={model.id}>
                              ...
                              <Button
                                onClick={() => handleDiscoveredModel(source, model)}
                                size="sm"
                              >
                                Use
                              </Button>
                  ...
                  <p className="model-muted-line">
                    ...
                    {selectedDiscoveredCount === 1 ? "" : "s"} for {selectedDefinition.label}.
                  </p>
                </section>
              ) : null}
            </div>
              ...
              <aside className="model-catalog-panel">
                ...
                <div className="model-catalog-list">
                  {visibleCatalog.length === 0 ? (
                    <p className="model-muted-line">Load the catalog to choose a model ID.</p>
                  ) : null}
                  {visibleCatalog.map((entry) => (
                    <div className="model-catalog-row" key={entry.id}>
                      ...
                      <Button onClick={() => handleCatalogEntry(entry)} size="sm">
                        Use
                      </Button>
                    </div>
                  ))}
