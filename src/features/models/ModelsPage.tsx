import { AlertTriangle, CheckCircle2, Database, Plus, Search, Trash2 } from "lucide-react";
...
export function ModelsPage({
  ...
  const [models, setModels] = useState<ModelRegistryRecord[]>(initialModels ?? []);
  ...
  const [discoverySources, setDiscoverySources] =
    useState<DiscoveredModelSource[]>(initialDiscoverySources);
  ...
  useEffect(() => {
    ...
    async function hydrateModels() {
      ...
      try {
        ...
        if (mounted) {
          setStatus(error instanceof Error ? error.message : "Unable to load model registry.");
        }
      } finally {
        if (mounted) {
          setLoading(false);
  ...
  async function handleSaveModel(input: ModelRegistryInput) {
    ...
    try {
      ...
      setStatus(result.action === "created" ? "Model registered." : "Model updated.");
      return true;
    } catch (error) {
  ...
  async function handleUpdateSelected() {
    ...
    try {
      const nextModels = models.map((model) =>
        model.id === selectedModel.id ? updateModelRegistryRecord(model, editInput) : model,
      );
  ...
  async function handleDeleteSelected() {
    ...
    const didConfirm =
      typeof window === "undefined" || window.confirm(`Delete model "${selectedModel.label}"?`);
  ...
  async function handleDiscover() {
    ...
    try {
      ...
      const modelCount = sources.reduce((count, source) => count + source.models.length, 0);
      setDiscoverySources(sources);
      setDiscoveryStatus(
        `${sourceCount} source${sourceCount === 1 ? "" : "s"}, ${modelCount} model${modelCount === 1 ? "" : "s"}.`,
      );
    } catch (error) {
      setDiscoveryStatus(error instanceof Error ? error.message : "Discovery failed.");
    } finally {
      setDiscovering(false);
    }
  }

  function handleFetchCatalog(input: ModelRegistryInput): Promise<ModelCatalogEntry[]> {
      ...
      <div className="page-stack">
        ...
        {models.length === 0 && !loading ? (
          ...
          <div className="models-grid">
            <Panel action={addModelButton} description={status} title="Model registry">
              <label className="models-search" htmlFor="model-search">
                <Search size={15} />
                <Input
                  id="model-search"
              ...
              {visibleModels.length === 0 ? (
                ...
                <table className="models-table">
                  <thead>
                    <tr className="models-table__header">
                      <th>Model</th>
                      <th>Provider</th>
                      <th>Target</th>
                      <th>Status</th>
                  ...
                  <tbody>
                    {visibleModels.map((model) => (
                      ...
                      >
                        <td className="models-table__name">
                          <strong>{model.label}</strong>
                          <small>{model.modelId}</small>
                        </td>
                        <td>{getModelProviderDefinition(model.providerKind).label}</td>
                        <td>{getModelTarget(model)}</td>
                        <td>
                        ...
                        <td>{formatRelativeTime(model.updatedAtIso)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {selectedModel && editInput ? (
                <div className="detail-stack">
                  ...
                  <div className="model-edit-form">
                    <label className="field-stack" htmlFor="edit-name">
                      <span>Name</span>
                      <Input
                        id="edit-name"
                        onChange={(event) =>
                    ...
                    <label className="field-stack" htmlFor="edit-model-id">
                      <span>Model ID</span>
                      <Input
                        id="edit-model-id"
                        onChange={(event) =>
                    ...
                    {editInput.providerKind === "manual-cli" ? (
                      <>
                        <label className="field-stack" htmlFor="edit-command">
                          <span>Command</span>
                          <Input
                            id="edit-command"
                            onChange={(event) =>
                        ...
                        <label className="field-stack" htmlFor="edit-args">
                          <span>Arguments</span>
                          <Input
                            id="edit-args"
                            onChange={(event) =>
                      ...
                      <label className="field-stack" htmlFor="edit-base-url">
                        <span>Base URL</span>
                        <Input
                          id="edit-base-url"
                          onChange={(event) =>
                  ...
                  <div className="property-grid">
                    <span>Provider</span>
                    <strong>{getModelProviderDefinition(selectedModel.providerKind).label}</strong>
                    <span>Method</span>
