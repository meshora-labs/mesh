...
import { getProjectStatusLabel, getProjectStatusTone } from "@/domain/status";
          ...
          <ul className="table-list">
            {visibleProjects.map((project) => (
              <li key={project.id}>
                <button
                  className={
                    project.id === selectedProject?.id ? "table-row is-selected" : "table-row"
                  }
                  onClick={() => onSelectProject(project.id)}
                  type="button"
                >
                  <span className="table-row__name">
                    {project.source === "git" ? <GitBranch size={16} /> : <FolderOpen size={16} />}
                    <span>
