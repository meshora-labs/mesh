...
export function CommandCenterPage({ workspace, projects, runs, loading }: CommandCenterPageProps) {
  ...
  return (
    <div className="page-stack">
      ...
      <div className="two-column-grid">
        <Panel description={workspace?.rootPath ?? "Local workspace"} title="Active projects">
        ...
        <Panel description="Execution states across local projects" title="Recent runs">
          <div className="dense-list">
            {runs.slice(0, 4).map((run) => (
              <div className="dense-row" key={run.id}>
                ...
                <Badge tone={getRunStatusTone(run.status)}>{getRunStatusLabel(run.status)}</Badge>
              </div>
            ))}
          </div>
        </Panel>
