...
function getModelFeatures(
	...
	const isHttpCompatible =
		providerKind !== "manual-cli" && providerKind !== "docker";
	const supportsJson =
		providerKind !== "manual-cli" && providerKind !== "docker";
...
export function WorkbenchPage({ agents, models, projects }: WorkbenchPageProps) {
	...
	const defaultModel =
		models.find((model) => model.status === "ready") ?? models[0] ?? null;
	...
	const [profileName, setProfileName] = useState(
		defaultAgent?.name ?? "Agent profile",
	);
	...
	}, [autoFollow, messages, logs, debugLogsOpen]);
	...
	function updateVariable(
		variableId: string,
		patch: Partial<Omit<WorkbenchVariable, "id">>,
	) {
		setVariables((currentVariables) =>
	...
	return (
		<div className="workbench-layout">
			<aside className="workbench-config-column" aria-label="Options and parameters">
				<Panel
					description="Agent, project, and provider contract"
					title="Agent profile"
				>
					<div className="workbench-form-grid">
						...
						<label className="workbench-field">
							...
							>
								{models.length === 0 ? (
									<option value="">No model registered</option>
								) : null}
								{models.map((model) => (
				...
				<Panel
					description={modelDefinition?.label ?? "No provider selected"}
					title="Generation"
				>
						...
						<div className="workbench-toggle-list">
							{functionOptions.map((option) => (
								<label className="workbench-check-row" key={option.id}>
									<input
										checked={
											features.functions && enabledFunctions.includes(option.id)
										}
			...
			<section className="workbench-console" aria-label="Message box">
				<header className="workbench-console__header">
					<div>
						...
						<p>
							{selectedModel?.modelId ?? "No model"} ·{" "}
				...
				<div className="workbench-telemetry" aria-label="Telemetry">
					<TelemetryMetric
							...
							>
								<header>
									...
									<time dateTime={message.createdAtIso}>
										{formatLogTime(message.createdAtIso)}
									</time>
								</header>
								<p>{message.content}</p>
							</article>
					...
					{debugLogsOpen ? (
						<div className="workbench-debug-log">
							<div className="workbench-subheader">
								...
								<Badge tone={storeLogs ? "live" : "warning"}>
									{storeLogs ? "stored" : "off"}
								</Badge>
							</div>
							{logs.length === 0 ? (
...
function SegmentedControl<T extends string>({
	...
	return (
		<div className="workbench-segment-field">
			<div className="workbench-subheader">
				<span>{label}</span>
				<Badge tone={disabled ? "warning" : "good"}>
