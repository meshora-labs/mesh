...
export function getModelProviderDefinition(providerKind: ModelProviderKind) {
	return (
		modelProviderDefinitions.find(
			(definition) => definition.kind === providerKind,
		) ?? modelProviderDefinitions[0]
	);
}

export function createDefaultModelInput(
	providerKind: ModelProviderKind,
): ModelRegistryInput {
...
export function upsertModelRegistryRecord(
	...
	const existingIndex = records.findIndex(
		(record) =>
			getModelRegistryIdentity(record) ===
			getModelRegistryIdentity(normalizedInput),
	);

	if (existingIndex >= 0) {
		const nextRecords = [...records];
		const record = updateModelRegistryRecord(
...
export function getModelRegistryIdentity(
	input: Pick<
		ModelRegistryInput,
		"providerKind" | "modelId" | "baseUrl" | "command" | "args"
	>,
) {
	return [
		input.providerKind,
